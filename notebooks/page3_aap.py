...
def extract_questions_with_uid(doc: Document):
    ...
    for para in doc.paragraphs:
        if "?" in para.text:
            ...
            para.add_run(f"\n{uid}")  # Insertion de l'UID
            results[uid] = [uid, question, size_info, '', '']  # [uid, question, format, contexte, réponse]
    return results, doc

# Page streamlit
def run():
    st.title("📄 Répondre à un Appel à Projet")
    st.write("Chargez un fichier `.docx` avec des questions. Les réponses seront générées automatiquement à partir des documents chargés en page 2.")

    if not api_key:
        st.error("Clé API OpenAI non définie. Veuillez configurer la variable d'environnement `OPENAI_API_KEY`.")
        return

    if "vector_store" not in st.session_state:
        st.warning("Aucun document de projet chargé. Rendez-vous d'abord dans la page 2.")
        return

    uploaded_file = st.file_uploader("📤 Charger le document AAP (.docx)", type=["docx"])
    ...
    if uploaded_file:
        ...
        # Traitement de chaque question avec RAG
        for uid, data in questions_dict.items():
            ...
            Utilise uniquement les documents fournis comme contexte pour répondre.
            Si tu ne trouves pas la réponse dans le contexte, réponds uniquement :
            **A REMPLIR PAR L'UTILISATEUR**
            ...
            Réponse :
            """)
            messages = prompt.format(question=question, context=context)
            response = llm.invoke(messages)

            rag_answer = response.content.strip()

            # traitement des questions sans réponse
            if not rag_answer or len(rag_answer) < 10 or "A REMPLIR" in rag_answer.upper():
                rag_answer = "**A REMPLIR PAR L'UTILISATEUR**"

            questions_dict[uid][4] = rag_answer

        # Remplacement des UID par les réponses dans le docx
        ...
        st.download_button(
            label="📥 Télécharger le document avec réponses",
            data=output,
