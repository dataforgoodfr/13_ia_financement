...
# ajout marine suite erreur

def get_vectorstore_from_hash(pp_hash: str):
    """
    Recharge une base FAISS existante à partir de son hash.

    Args:
        pp_hash (str): hash du projet (clé utilisée dans pp_hashes.json)

    Returns:
        FAISS | None : la base vectorielle FAISS si elle existe, sinon None
    """
    db_path = os.path.join("vector_stores", pp_hash)
    if not os.path.exists(db_path):
        print(f"[get_vectorstore_from_hash] ❌ Chemin non trouvé : {db_path}")
        return None

    try:
        embeddings = embedding_model  # déjà défini plus haut
        faiss_db = FAISS.load_local(
            folder_path=db_path,
        ...
        print(f"[get_vectorstore_from_hash] ❌ Erreur lors du chargement : {e}")
        return None
